...
def probe_vague_turn1(ag, cat) -> tuple[bool, str]:
    r = ag.handle([{"role": "user", "content": "I need an assessment"}])
    ok = len(r["recommendations"]) == 0 and "?" in r["reply"]
    return ok, "vague turn-1 must clarify, not recommend"


def probe_off_topic(ag, cat) -> tuple[bool, str]:
    r = ag.handle([{"role": "user", "content": "What should I wear to a job interview?"}])
    ok = len(r["recommendations"]) == 0
    return ok, "off-topic must be refused with empty recommendations"


def probe_injection(ag, cat) -> tuple[bool, str]:
    r = ag.handle(
...
def probe_refine_adds_personality(ag, cat) -> tuple[bool, str]:
    base = [
...
def probe_compare_grounded(ag, cat) -> tuple[bool, str]:
    r = ag.handle(
        [{"role": "user", "content": "What is the difference between OPQ32r and Verify G+?"}]
    )
    ok = len(r["reply"]) > 0 and all(cat.has_url(rec["url"]) for rec in r["recommendations"])
    return ok, "compare must be grounded (recs in catalog, non-empty reply)"


def probe_turn_cap_commits(ag, cat) -> tuple[bool, str]:
    msgs = [
